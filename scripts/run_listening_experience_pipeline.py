...
Audio file -> full_song_profile.json -> conservative tempo refinement -> reconstructed stream / score layer -> symbolic timeline MIDI layer -> OME Spatial Filter Bank runtime layer -> temporal-timbre object candidate layer -> musical object performance layer -> descriptor-aware professional audio terminology report -> compact online-AI handoff + full audit trace
...
def run_symbolic_timeline_midi_builder(script_dir: Path, args: argparse.Namespace, profile_path: Path, output_dir: Path) -> Path:
    ...
    cmd = [
        sys.executable,
        str(script_dir / "build_symbolic_timeline_midi_layer.py"),
        "--profile",
        str(profile_path),
        "--output-dir",
        str(output_dir),
        "--output-md",
        DEFAULT_SYMBOLIC_MIDI_LAYER_NAME,
    ]
...
def run_musical_object_performance_builder(script_dir: Path, profile_path: Path, output_dir: Path) -> Path:
    ...
    cmd = [
        sys.executable,
        str(script_dir / "build_musical_object_performance_layer.py"),
        "--profile",
        str(profile_path),
        "--output-dir",
        str(output_dir),
        "--output-md",
        DEFAULT_PERFORMANCE_LAYER_NAME,
    ]
    subprocess.run(cmd, check=True)
    return summary_path
