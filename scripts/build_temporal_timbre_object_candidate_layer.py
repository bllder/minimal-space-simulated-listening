...
def build_layer(profile: dict[str, Any], external_packets: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "candidate_generation_rule": "Object-family candidates are formed from professional-term-anchored time-frequency-timbre continuity, source/effect-family hypotheses, optional external evidence, and optional OME mapping support. They are not confirmed source-separated stems, true instrument identities, or exact effect-chain claims.",
        "evidence_sources": {"profile_segments": len(segments), "external_adapter_packet_count": len(external_packets), "ome_mapping_status": ome_layer.get("status") or "not_attached"},
        ...
        "truth_boundary": "MSSL object candidates are professional-term-anchored listening objects, not original stems, confirmed instruments, confirmed effects chains, singer identity, lyric truth, genre truth, or emotion truth.",
    }
...
def build_candidate(family_id: str, spec: dict[str, Any], segments: list[dict[str, Any]], ome_layer: dict[str, Any], external_index: dict[str, Any]) -> dict[str, Any]:
    segment_values = []
    ...
    claim_strength = claim_strength_for(support_summary, temporal, timbre, source_family, spec)
    ...
    return {
        ...
        "object_continuity_card": {"formation_chain": formation_chain, "continuous_object_sentence": continuous_sentence, "handoff_sentence": continuous_sentence, "why_not_source_truth": "This is a professional-term-anchored listening-object candidate, not a confirmed source-separated stem, instrument identity, performer action, sample source, or exact effect chain."},
        "evidence": {"temporal_continuity": temporal, "timbre_continuity": timbre, "spectral_envelope_support": spectral, "pitch_or_contour_support": contour, "source_family_support": source_family, "ome_mapping_support": ome_mapping},
...
def should_keep_candidate(candidate: dict[str, Any]) -> bool:
    ...
    return to_float(summary.get("max_support")) >= 0.38 or candidate.get("claim_strength") in ("medium", "strong")
...
def build_formation_chain(spec: dict[str, Any], support: dict[str, Any], temporal: dict[str, Any], timbre: dict[str, Any], spectral: dict[str, Any], contour: dict[str, Any], source_family: dict[str, Any], ome: dict[str, Any], terms: list[dict[str, str]]) -> list[dict[str, Any]]:
    return [
        ...
        {"step": "source_or_effect_family_boundary", "value": source_family.get("full_mix_source_hint_counts") or source_family.get("external_adapter_support"), "meaning": "Source/effect-family evidence is a bounded source-family grouping hypothesis and must not be promoted to source truth."},
